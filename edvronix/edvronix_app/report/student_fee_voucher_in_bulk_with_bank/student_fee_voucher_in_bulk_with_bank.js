...
frappe.query_reports["Student Fee Voucher in Bulk With Bank"] = {
	...
	onload: function (report) {
		...
		report.page.add_inner_button(__("Print Vouchers"), function () {
...
function generate_print_html(data, filters) {
	...
	Object.values(grouped).forEach(parent => {
		...
		parent.rows.forEach(r => {
			...
			let paid = flt(r.paid_amount || 0);
			let outstanding = flt(r.outstanding || 0);
			let invoice_total = paid + outstanding;

			total_val += invoice_total;

			if (!r.due_date) return;

			let due = frappe.datetime.str_to_obj(r.due_date);

			// Calculate month difference
			let month_diff = (today.getFullYear() - due.getFullYear()) * 12 +
				(today.getMonth() - due.getMonth());

			// CONDITION 1: Partial Payment → arrears
			if (paid > 0 && outstanding > 0) {
				arrear_val += outstanding;
				return;
			}

			// CONDITION 2: Older than 2 months unpaid → arrears
			if (outstanding > 0 && month_diff > 2) {
				arrear_val += outstanding;
				return;
			}
		...
		["Bank Copy", "Office Copy", "Parent Copy"].forEach((copy_name) => {
			...
			<div class="voucher-wrapper">
				...
				<div class="student-table-container">
					<table>
						<thead>
							<tr>
								<th style= "width: 37%;">Student / Class / Section</th>
			...
			parent.rows.forEach(r => {
                // Ensure invoice_items is processed correctly
                let items_html = "";
                if (r.invoice_items) {
                    items_html = r.invoice_items.split(',').map(item => item.trim()).join('<br>');
                } else {
                ...
                <tr>
                    <td>
                        ...
                        <small>${r.program || "General"}</small> | <small>${r.student_category || "General"}</small>
                    </td>
                    <td>
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <b>${r.invoice_id || "N/A"}</b>
                            <small><b><i>${r.status || "N/A"}</i></b></small>
                        </div>
                        <div style="font-size: 8.5px; color: #4a5568; margin-top: 4px; border-top: 0.5px solid #eee; padding-top: 2px; line-height: 1.3;">
                            ${items_html}
                        </div> 
                    </td>                    
